...
class Speaker(object):
    ...
    def wake(self, bot, wake=None):
        if wake and isinstance(wake, bool):
            # Se wake é True, usar mensagem padrão
            wake_message = "Good morning. I just woke up"
        elif wake and isinstance(wake, str):
            # Se wake é uma string, usar ela
            wake_message = wake
        else:
            # Se wake é False/None, não enviar nada
            return
        
        send(bot, self.admin, wake_message)
